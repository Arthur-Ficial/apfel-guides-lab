"""Ruby language scripts - run each against live apfel --serve."""
import json
...
RB_DIR = REPO_ROOT / "scripts" / "ruby"


def _run(script: str, stdin: str | None = None):
    return run_script(
        ["bundle", "exec", "ruby", str(script_path("ruby", script))],
        cwd=RB_DIR, stdin=stdin,
    )


def test_oneshot(capture_mode):
    r = _run("01_oneshot.rb")
    assert_model_output_ok(r)
    if capture_mode:
        capture_output("ruby", "01_oneshot.rb", r.stdout)
...
def test_stream(capture_mode):
    ...
    assert "\n" in r.stdout
    if capture_mode:
        capture_output("ruby", "02_stream.rb", r.stdout)
...
def test_json(capture_mode):
    ...
    if capture_mode:
        capture_output("ruby", "03_json.rb", r.stdout)
...
def test_errors(capture_mode):
    ...
    if capture_mode:
        capture_output("ruby", "04_errors.rb", r.stdout)


def test_tools(capture_mode):
    r = run_until_weather_answer(
        ["bundle", "exec", "ruby", str(script_path("ruby", "05_tools.rb"))],
        cwd=RB_DIR,
    )
    assert_model_output_ok(r)
    assert any(w in r.stdout.lower() for w in WEATHER_KEYWORDS), r.stdout
    if capture_mode:
        capture_output("ruby", "05_tools.rb", r.stdout)
...
def test_example(capture_mode):
    r = _run("06_example.rb", stdin=FIXTURE_TEXT)
    assert_model_output_ok(r, min_chars=50)
    if capture_mode:
        capture_output("ruby", "06_example.rb", r.stdout)
